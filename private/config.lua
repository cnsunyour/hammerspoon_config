...
hs.hotkey.bind({ "ctrl", "shift", "cmd" }, "M", function()
    local theapp = hs.application.get('Emacs')
    if (theapp ~= nil and theapp:mainWindow() and theapp:isFrontmost()) then
        theapp:hide()
    else
        hs.application.launchOrFocus("/opt/homebrew/opt/emacs-plus@30/Emacs.app")
    end
end)

hs.hotkey.bind({ "ctrl", "shift", "cmd" }, "N", function()
    local theapp = hs.application.get('Cursor')
    if (theapp ~= nil and theapp:mainWindow() and theapp:isFrontmost()) then
        theapp:hide()
    else
...
hs.hotkey.bind({ "ctrl", "shift", "cmd" }, ",", function()
    local theapp = hs.application.get('WezTerm')
    if (theapp ~= nil and theapp:mainWindow() and theapp:isFrontmost()) then
        theapp:hide()
    else
